...
class TableManager
{
    ...
    {
        $this->config = Config::get('table-manager');
    ...
    public function index($viewName)
    {
        ...
        //// setup
        $locales = $this->locales_table ? DB::table($this->locales_table)->select('id', 'code')->orderBy('id', 'ASC')->get() : [];
        $subItems = [];
        ...
        if (count($this->translate_form_input_control) >= 1) {
            if (Session::has('locale')) {
                Session::put('locale', $this->default_locale);
            } else {
                Session::put('locale', $this->default_locale);
            }
...
//                    }
        ...
        if (count($this->multi_items_form_input_control) >= 1) {
            ...
            Session::put('first_id', $e_pre[$this->save_first_id_column]);
        }
        return $e_data;
